...
- Use available integrations or read-only CLI requests, such as `gh pr view`, `gh pr diff`, `gh pr checks`, `glab mr view`, `glab mr diff`, `glab ci status`, and read-only `gh api` / `glab api` requests. Target the item's URL and repository explicitly; the current local checkout may be unrelated or a different revision.
